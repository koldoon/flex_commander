...
abstract class FsNode {
  ...
  /// Глубина строки в списке вида и раскрытость ветви. Свойства **показа**:
  /// их проставляет маппер вида, узел о них ничего не решает
  /// (`spec/panel-node-list.md`, §4). Здесь, а не в подтипе узла: подтип
  /// перестал бы быть каталогом или ссылкой, и проверки типа сломались бы.
  int get level;
  bool get isOpen;
...
поэтому курсор переносится по имени, а пометка — по пути (`pathString`): имя
опознаёт объект внутри каталога, а помечать умеют и из соседних ветвей дерева.
Это явное правило, см. [`state.md`](state.md#перечитывание-каталога).
...
class DirectoryNode extends FileNode {
  ...
  /// Порядок — как вернул провайдер; сортировка живёт в PanelSession.
...
| `realFileSystem` | команда «открыть системой» и запуск: путь уходит внешней программе как есть, и он должен быть настоящим |
...
Ход работы держит **объект** (`OperationStatus`), а не событие: событие можно
пропустить, объект нельзя. Что именно про работу известно — доля, скорость,
байты, плечи, — говорят его подтипы, и работа объявляет о себе тем, что
реализует. Отдельного отчёта-события у контракта нет.

```dart
enum OperationState { inited, pending, processing, userActionRequired, complete, canceled, error }

abstract class Operation<P, R> {
  /// Ход работы — объект, и другого канала нет: событие можно пропустить,
  /// объект нельзя. Что про работу известно — доля, скорость, байты, этапы, —
  /// говорят подтипы OperationStatus.
...
это «0 из 1» до самого конца. Поэтому статус несёт ещё и объём:
`bytesTransferred`, `bytesTotal`, `speed` и производную от них оценку
`remaining`.
...
Статус несёт не только долю, но и оба числа, из которых она получается:
`itemsTransferred` (обработано) и `itemsTotal` (всего). Доля — производная, потому
что честный процент требует знать общее количество, а узнать его для дерева дорого:
...
Сортировка выполняется в `PanelSession` после чтения каталога и при смене `SortSpec`;
...
(`PanelSession._cursorMemory`), и открытие сохранённого каталога после запуска
...
```dart
/// 126 -> "126", 92262 -> "90.1K", 2097152 -> "2.0M", -1 -> "" (размер неизвестен).
/// Основание 1024; начиная с K — **всегда** один знак после запятой, даже
/// нулевой: растущая сумма обходимого каталога иначе дёргается формой и шириной
/// на круглом значении (`71.9M` -> `72M` -> `72.1M`). Для байт — без суффикса.
