...
разбор пути рекурсивен, и вложенных по дороге бывает несколько. Приём тот же, что
у составных команд (`CommandExecutorBase._active` / `cancelActive`) — прерывают
всегда снаружи, а работает самая вложенная, и добраться до неё можно только так.
...
- **байты растут внутри файла**: движок отмечает каждый кусок потока, а не
  только законченный файл. Там, где байты идут мимо него (провайдер копирует
  файл сам, переименование переносит поддерево разом), объём засчитывается
  целиком по размеру объекта;
