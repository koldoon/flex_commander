...
| [`spec/client-server.md`](spec/client-server.md) | Разделение на ядро и интерфейс: границы, протокол, порядок переезда |
...
**Ядро и интерфейс друг друга видеть не должны**, и следить за этим будет
компилятор: зависимости `fc_ui_api` на `fc_core_api` не станет, как только
узловые подписи уйдут из `Panel` (`spec/client-server.md`, Э3). Пока она есть и
объявлена временной — прямо в `pubspec.yaml`, с перечнем того, что её держит.

```
        ┌───────────────────────────────────────────────┐
        │ flex_commander                                │
        │ ядро в изоляте · интерфейс · линк между ними   │
        └───────────┬───────────────────────┬───────────┘
                    │                       │
      ┌─────────────▼──────────┐ ┌──────────▼─────────────┐
