...
- **Ф13.** Найдена и закрыта дыра сборки: **модуль, указанный дважды, устанавливался
  дважды** и удваивал свои привязки. Всплыло на голдене справки — там появилось «F1, F1».
  Теперь `installAll` пропускает модуль с уже занятым именем; на это есть тест.
  Голден справки из-за этого перегенерирован второй раз — первый (в Ф12) закрепил как раз
  удвоение.
- **Ф13.** `LegacyCommands` и `default_commands.dart` удалены. Справка и заглушки
  `app.menu`/`file.view`/`file.edit` переехали в `AppShell`: это не «ещё не разъехавшийся
  набор», а собственная оболочка приложения.
- **Ф13.** Доктринальные тесты: чистота `fc_api` от `dart:io` и виджетов (Ф3),
  «ядро знает только `fc_api`» и «ядро не лезет в `package:fc_api/src/`» (`test/app/
  layering_test.dart`), «каждая привязка указывает на установленную команду»
...
- **Значения оформления уехали из `fc_api` в `fc_default_theme`.** В API остались роли:
  `FcColors`, `FcMetrics`, `FcIcons`, `FcFonts` — абстрактные, без единого числа и цвета.
  Следствия, которые из этого вытекли:
  - тема стала **обязательной**: без неё сборка не начинается, как и без корневого
    источника дерева. Раньше приложение без модуля темы работало на встроенных
    умолчаниях API — теперь красить нечем;
  - `FcThemeSpec.fallback` исчез, а `colors`/`metrics`/`icons`/`fonts` в спеке стали
    обязательными;
  - `testApp` подставляет `DefaultTheme` так же, как оболочку и платформу;
  - тесты, закреплявшие палитру и кегль, переехали в `fc_default_theme`: проверка живёт
    рядом со значениями;
  - тесты набора элементов в `fc_api` рисуют его оформлением по умолчанию —
    у пакета появилась dev-зависимость на модуль темы. Своих значений у него нет и
    не должно быть, а рисовать тесту чем-то надо.
  - вторая тема наследуется от `DefaultColors`/`DefaultMetrics` и переопределяет нужное:
    переписывать полсотни размеров не приходится.

## Что осталось за рамками

- `CommandFlow` со связями и условиями — ветвления по итогу шага. Портировать его
  сейчас значило бы строить каркас под несуществующую нагрузку.
- Список команд (палитра) и переназначение клавиш из настроек: механизм готов —
  привязки живут в реестре и заменяются по идентификатору, — но интерфейса к нему нет.
- Просмотрщик (F3) и редактор (F4) — пока заглушки в оболочке: клавиша закреплена,
  кнопка приглушена.
- Светлая тема: контракт отлажен на `fc_default_theme`, второй теме остаётся объявить
  свой `FcThemeSpec`.
- Тесты моделей всё ещё живут в приложении: они опираются на `fc_test_kit`, который
  зависит от ядра. Разъедутся, когда подставки перестанут нуждаться в `AppController`.
