...
- обход **перезапускается** при каждом изменении пометки, но с паузой в 200 мс:
  `Space` нажимают очередями, `Cmd-A` помечает всё разом, и начинать обход на каждое
  нажатие незачем — он всё равно был бы отменён следующим;
