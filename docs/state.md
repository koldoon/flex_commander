...
размер. Размер файлов известен из листинга сразу; каталоги приходится обходить, и
обход большого дерева — работа надолго, поэтому:

- он идёт **фоном**, а панель тем временем показывает то, что уже известно: сумму
  файлов и то, что успело насчитаться в каталогах;
- пока обход не закончен, к строке добавляется `(Scanning…)` — иначе растущее число
...
- новые каталоги **встают в очередь**, а не прерывают начатое: помечать продолжают
  ...
  дойдёт никогда. Снятая пометка убирает каталог отовсюду — из суммы, из очереди, а
  если считают именно его, обход прекращается и очередь идёт дальше;
- посчитанное запоминается по имени каталога, поэтому повторная пометка того же
  каталога обходится без нового обхода;
