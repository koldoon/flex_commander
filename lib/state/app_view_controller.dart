...
class AppViewController extends ChangeNotifier implements ApplicationView {
  ...
  bool takesKeys(ViewportState content) {
    ...
    // Полоса под активной панелью, забравшая набор себе, — быстрый поиск.
    //
    // Областью она не бывает и активной не станет никогда, но клавиши в этот
    // момент у неё, и показать это должна она. Спрашивается признак самого
    // содержимого ([ViewportState.takesKeyboard]), а не «поиск ли это»: ядро о
    // модуле навигации не знает, а полоса хода работы клавиш не берёт и
    // светиться не должна.
    //
    // Полос под панелью две — своя у каждой стороны, — и без этого условия обе
    // выглядели одинаково: две обводки и два курсора на экране, а набор идёт
    // ровно в одну.
    final status = active.status;
    final under = status == null ? null : contentAt(status);
    if (under != null && under.takesKeyboard && _holds(under, content)) {
      return true;
    }
