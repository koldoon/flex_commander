...
class AppController extends ChangeNotifier implements Application {
  ...
  /// Перерисовать экран.
  ///
  /// Разделы модулей правятся мимо контроллера (`moduleSettings`), и о правке
  /// он не узнаёт: без толчка выключенный ряд наборов пропал бы только при
  /// следующей чужой перерисовке. Тем же приёмом живёт смена языка —
  /// `StringsRegistry.refresh`.
