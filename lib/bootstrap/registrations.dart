...
class LazyServices implements FcServices {
  ...
  /// Сколько есть — ноль тоже ответ.
  ///
  /// Спрашивается **объявлено ли**, а не ловится отказ. О незнакомом типе
  /// контейнер сообщает исключением, а исключение, вылетевшее из фабрики,
  /// сбивает ему разбор зависимостей: он очищает стопку разбора, и падает уже
  /// следующий вопрос — `RangeError` в чужом месте. Поймать это можно, а
  /// починить нельзя, поэтому и не спрашиваем.
  ///
  /// Так спрашивают о необязательном: нет модуля типов — правила по содержимому
  /// не совпадают; нет миниатюр — значок берётся следующим правилом.
  @override
  List<T> resolveAll<T>() => _declared<T>() ? _di.getAll<T>() : const [];

  /// Объявлена ли служба — здесь или выше по цепочке контейнеров.
  bool _declared<T>() {
    for (DI? at = _di; at != null; at = at.parent) {
      if (at.bindings.containsKey(T)) {
        return true;
      }
    }
    return false;
