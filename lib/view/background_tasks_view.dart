...
///
/// **Прерывание молчаливое.** Нажатый крестик и есть ответ: человек смотрит на
/// список работ, целится в строку и жмёт крестик именно на ней — переспрашивать
/// после этого значит спрашивать дважды. Так же ведёт себя «Стоп» в окне
/// поиска, и по той же причине.
///
/// Прежде крестик **просил** прерваться, а работа переспрашивала — и ради
/// вопроса ей возвращалось окно. Живьём это выглядело так, что фоновый поиск
/// крестиком не остановить вовсе: вместо остановки открывалось окно находок.
/// Вопрос при этом остаётся там, где он к месту: `Esc` в самом окне работы
/// по-прежнему спрашивает — рефлекс и прицельный щелчок это разные вещи.
///
...
void cancelOrForgetTask(Operations operations, OperationRun task) {
  ...
  // И строка уходит: целились в неё, чтобы работы не стало, — а «прервана» в
  // списке живых работ не живая и не нужная.
  operations.forget(task.runId);
}
