...
class PanelSession {
  ...
  void toggleCurrentMark({bool step = true, int seq = 0, int cursorSeq = 0}) {
    ...
    // Шаг — **первым**, пометка — следом, по уже запомненному узлу.
    //
    // Порядок не косметический: пометка уведомляет о себе, и каждое
    // уведомление уносит состояние за границу. Сделай её первой — и наружу
    // уедет состояние со **свежим номером заявки и вчерашним курсором**; та
    // сторона примет его за подтверждение и вернёт курсор на строку назад.
    // Живьём это и был тот дёрганый курсор при обсчёте размеров: щуп показал
    // «шлю состояние: курсор=15 seq=320», а через миллисекунду «курсор=16
    // seq=320» (разбор 23 сентября 2026, `docs/spec/client-server.md`, §5.5).
    if (step) {
      moveCursor(1);
    }
