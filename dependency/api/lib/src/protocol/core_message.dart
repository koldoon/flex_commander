...
/// У посчитанных каталогов появился размер: путь → новое число.
///
/// Отдельно от списка, и это не мелочь: обход помеченного меняет по одному
/// числу, а список бывает в десять тысяч строк.
///
/// Адрес здесь — **путь**, а не номер строки, и это единственный способ, каким
/// размер вообще может ездить. Размер принадлежит каталогу, а не месту в
/// списке: список перечитывается на каждый шаг курсора по дереву, узлы
/// становятся новыми, и всякая привязка к строке после первого же чтения либо
/// умирает, либо садится на чужую строку — оба раза это было видно живьём
/// (`docs/spec/directory-sizes.md`).
///
/// [FsNode.unknownSize] значит «забудь»: обход оборвали, и частичная сумма,
/// застывшая в колонке, была бы ложью.
final class PanelSized extends CoreEvent {
  const PanelSized(this.panel, this.sizes);

  final PanelId panel;

  /// Путь каталога — его размер.
  final Map<String, int> sizes;
}
