...
///
/// Путём, а не номером. Номер годится, пока список меняется только по просьбе
/// этой же стороны: вид «Столбцы» меняет его **сам** — придержка раскрывает
/// каталог под курсором, — и заявка, посчитанная на прежних строках,
/// применялась уже к новым. Живой разбор 17 сентября 2026: нажали `Down` на
/// `/opt`, заявка ушла с номером соседа, а доехала в список, где под этим
/// номером уже стоял `/opt/homebrew`, — курсор проваливался внутрь только что
/// раскрытого каталога (`docs/spec/panel-view-columns.md`, §5).
///
...
final class ToggleMark extends CoreRequest implements PanelRequest {
  ...
  ///
  /// Зеркало показывает свою пометку **сразу**, не дожидаясь ответа: пока
  /// ядро занято обходом каталогов, ответ отстаёт, и без этого пометка
