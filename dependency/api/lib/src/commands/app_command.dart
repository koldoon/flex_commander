...
abstract class AppCommand extends ChangeNotifier {
  ...
  /// Описание окна команды; null — окно не нужно.
  ///
  ...
  ///
  /// Одним объектом, а не набором геттеров: окно — это его содержимое вместе с
  /// местом и тем, берёт ли оно фокус, и разносить это по трём разным местам
  /// значит однажды поменять одно и забыть другое.
  ///
  /// Окно — надстройка: оно задаёт те же параметры и вызывает тот же [execute].
  DialogSpec? dialogSpec(BuildContext context) => null;
