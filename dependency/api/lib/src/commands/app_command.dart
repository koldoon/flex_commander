...
/// контейнер.
///
/// Не путать с `CommandFactory` командного фреймворка: та создаёт любую
/// команду и получает данные составной команды, а эта — именно действие
/// приложения, со своим идентификатором, названием и окном.
...
///
/// **Действие приложения — это [Command] командного фреймворка.** Сигнатура
/// [execute] у них совпадает, поэтому любое действие можно поставить в
/// последовательность или в параллель наравне с чем угодно другим: на этом
/// вырастут макросы, палитра команд и сценарии. Всё, что здесь добавлено
/// сверх [Command], — приложению: идентификатор, название, окно, условия
/// запуска.
abstract class AppCommand extends ChangeNotifier implements Command {
