...
class NodePath {
  ...
  ///
  /// Правило одно, и оно про то, откуда путь начинается:
  ///
  /// * локальная файловая система своего имени не называет — путь начинается
  ///   с косой черты: `/Users/koldoon`;
  /// * любой другой источник называет протокол обязательно, иначе непонятно,
  ///   о какой машине речь: `ssh://koldoon@shark/etc`;
  /// * всё, что смонтировано **по дороге** (архив), протоколом не называется
  ///   вовсе и выглядит каталогом: `/Users/koldoon/some.zip/folder`,
  ///   `ssh://koldoon@shark/etc/backup.zip/squid`.
  ///
  /// Разница между первой частью и остальными не произвол: начало пути
  /// отвечает на вопрос «где», а промежуточное звено — только на вопрос «что
  /// внутри». Внутрь архива пользователь входит как в каталог и путь ожидает
  /// увидеть такой же.
  ///
  /// Обратно такая строка **однозначно** не разбирается: по ней не видно, где
  /// кончается файл архива и начинается путь внутри него. Узнать это можно
  /// только спросив источник о типе каждого звена — этим занимается
  /// `ProviderRegistry.resolveDisplayPath`. Там, где спрашивать не хочется
  /// (настройки), по-прежнему хранится [toString] — со схемами.
  String get displayString {
    final first = parts.first;
    // Схема по умолчанию — это «схемы не было»: локальный путь выглядит
    // обычным путём. Чужая схема остаётся: `//koldoon@shark` без неё — ни
    // адрес, ни путь.
    final buffer = StringBuffer(
      first.scheme == defaultScheme ? _trimSlash(first.path) : '${first.scheme}:${_trimSlash(first.path)}',
    );
