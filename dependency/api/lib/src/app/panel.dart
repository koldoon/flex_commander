...
abstract interface class Panel implements ViewportState {
  ...
  /// Набранное в быстром поиске; null — режим выключен.
  ///
  /// Состояние панели, а не команды: образец относится к **этому** списку, и
  /// уход в другой каталог его отменяет. Показывает его строка состояния, а
  /// ведёт курсор за ним тот, кто разбирает клавиши.
  ///
  /// Пустая строка — режим включён, но ничего не набрано: так он выглядит сразу
  /// после `Ctrl-S`, и это не то же самое, что выключенный.
  String? get quickSearch;

  /// Включить, уточнить или выключить быстрый поиск (null — выключить).
  ///
  /// Курсор эта запись не двигает: куда ему идти, решает тот, кто набирает, —
  /// правило поиска (с начала имени, от курсора, по кругу) принадлежит команде,
  /// а не панели.
