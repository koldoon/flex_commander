...
class UpdateService {
  ...
  ///
  /// Оно одно на приложение: две проверки подряд приводят к двум загрузкам
  /// одного и того же выпуска, а это одна работа, один файл и одно окно
  /// «готово» (`docs/spec/self-update.md`, §4).
  bool get busy => _busy;

  /// Занять службу на время одного обновления.
  ///
  /// Здесь, а не у команды: служба одна, а зовущих двое — проверка при запуске
  /// и «Проверить сейчас», — и знать друг о друге они не обязаны.
  Future<void> hold(Future<void> Function() work) async {
    _busy = true;
    try {
      await work();
    } finally {
