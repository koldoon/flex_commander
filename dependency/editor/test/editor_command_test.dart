...
void main() {
  ...
  Future<void> waitUntil(bool Function() done) async {
    // Обороты очереди **и** настоящее время: запись идёт на диск, и обороты её
    // не ускоряют — сколько ни крути, файл пишется столько, сколько пишется.
    // Одними оборотами прогон под нагрузкой изредка не дожидался конца.
