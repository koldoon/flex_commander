...
class FcTheme extends ThemeExtension<FcTheme> {
  ...
  ///
  /// Всё три порознь брать нельзя, и каждое отвечает за своё.
  ///
  /// **Семейство и запасные.** Шрифт списка берётся из системы, и на машине,
  /// где его нет, подстановку выбирает тот, кто рисует: Flutter возьмёт свою,
  /// `xterm` — свою, и один и тот же текст выйдет разными шрифтами.
  ///
  /// **Межстрочная.** У `xterm` она своя и всегда — [terminalLineHeight];
  ...
  ///
  /// Число не наше: столько ставит `xterm` (`TerminalStyle.height`), а
  /// подстроиться проще нам — у терминала строка обязана попадать в клетку, а
  /// у остальных нет.
