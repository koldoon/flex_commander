...
///
/// [topInset] — сколько над окном: все окна стоят на одном расстоянии от
/// верхнего края (`FcMetrics.dialogTopInset`), и считать их высоту от середины
/// нельзя. Переросшее свой предел окно раскладка подняла бы выше назначенного
/// — то самое дрожание, ради избавления от которого его туда и прибили.
BoxConstraints dialogContentLimits(BuildContext context, {bool titled = true, double? topInset}) {
  ...
  final height = screen.height - (topInset ?? inset) - inset - (titled ? metrics.dialogTitleHeight : 0);
