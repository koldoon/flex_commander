...
class DialogSpec {
  ...
  ///
  /// **Имя окна, а не имя размера.** Размер — первое, что понадобилось
  /// запоминать, но вряд ли последнее: завтра это будет разложение колонок в
  /// находках, послезавтра — открытая вкладка. Поле, названное под один
  /// случай, пришлось бы переименовывать вместе со вторым, а имя это внешний
  /// контракт — оно лежит в `settings.json`
  /// (`docs/spec/dialog-resize.md`, §3).
  ///
  /// Обычно совпадает с идентификатором команды, которая окно показала.
  /// null — окно безымянно: помнить о нём нечего и негде.
  final String? id;

  /// Окно тянется за края и углы.
  ///
  /// Отдельно от [id], потому что это разные вещи: окно может помнить о себе
  /// что-то, не меняя размера. Тянуть можно и безымянное — только размер не
  /// переживёт закрытия.
  ///
  /// Умение объявляется, а не подразумевается: у формы с выверенной по
  /// референсу раскладкой растяжение разъедет столбцы, а у вопроса
  /// «перезаписать?» тянуть нечего (`docs/spec/dialog-resize.md`, §4).
  ...
  /// Назначает ли окно ширину само.
  ///
  /// Обычное окно облегает содержимое, и рама держит его в пределах из темы
  /// (`dialogMinWidth`/`dialogMaxWidth`): у справки с длинными описаниями
  /// собственная ширина ушла бы за край экрана. Окно, которое берёт ширину
  /// долей экрана, в этой опеке не нуждается — предел у него свой и в тех же
  /// долях, а верхний предел рамы обрезал бы его на широком экране тем
  /// сильнее, чем экран шире. Так устроена палитра команд.
  final bool ownWidth;

  /// Облегает ли окно **над панелью** своё содержимое.
  ///
  /// Обычное окно над панелью берёт ширину своей области: содержимое у таких
  /// окон меняется на глазах — бегут имена файлов, список отбирается по
  /// набранному, — и облегающее окно дёргалось бы шириной на каждом шаге
  /// (`docs/spec/dialog-placement.md`, §3).
  ///
  /// Там, где содержимое стоит на месте, это правило только вредит: окно
  /// растягивается во всю панель, и связанные вещи разъезжаются по краям.
  /// Такое окно облегает содержимое — но не шире своей области: границы
  /// остаются те же.
