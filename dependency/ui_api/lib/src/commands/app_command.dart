...
class DialogSpec {
  ...
  /// Где окно стоит по вертикали.
  ///
  /// По умолчанию посередине — окно спрашивает и уходит, и середина для этого
  /// самое спокойное место. Палитра команд стоит иначе, и не ради вида: её
  /// содержимое меняется прямо во время набора — находок то больше, то
  /// меньше, — а окно, растущее и сжимающееся вокруг своей середины, дёргает
  /// **обе** свои границы. Прибитое к верху, оно двигает только нижнюю, а поле
  /// ввода, в которое человек в этот момент смотрит, стоит на месте.
...
/// Где окно команды стоит по вертикали.
///
/// Расстояние сверху назначает не окно, а тема (`FcMetrics.dialogTopInset`):
/// это оформление, и в одном месте его видно целиком.
enum DialogPlacement {
  /// Посередине по высоте — обычное окно.
  center,

  /// Под верхним краем, на отступе из темы.
  top,
}
