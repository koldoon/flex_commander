...
void main() {
  ...
  group('канал данных', () {
    ...
    test('канал данных закрывается до чтения хвоста, а не после', () async {
      // Сервер досылает `226`, когда закрылись **обе** стороны. Клиент,
      // который держит свою половину открытой, ждёт сервера до его таймаута —
      // и каталог из шести строк открывается те же десять секунд, что и
      // каталог из ста. Ровно это и выглядело как «втрое медленнее Finder»
      ...
      expect(
        watch.elapsed,
        lessThan(FakeFtpServer.lingerGuard),
        reason: 'ждали сервер вместо того, чтобы закрыть свою половину',
      );
    });
