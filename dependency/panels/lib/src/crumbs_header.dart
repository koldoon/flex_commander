    ...
    return Row(
      ...
      children: [
        for (var i = 0; i < shown.length; i++) ...[
          ...
          // Звенья гнутся, хотя свёртка уже отмерена по ширине: ширину слота
          // плашка знает **оценкой** (`HistoryArrows.widthOf`), а не замером,
          // и погрешность в пару точек прежде съедал гибкий текст пути. Без
          // этого она вылезает за плашку переполнением (поймано тестом узкой
          // панели).
          if (shown[i] case final crumb?)
            Flexible(
              // Последнее — ярко, всё до него приглушённо: строки различаются
              // концом (§4).
              child: _Crumb(crumb: crumb, panel: view.panel, style: i == shown.length - 1 ? view.style : dim),
            )
          else
