...
class _FileTableState extends State<FileTable> {
  ...
  /// **Помеченного в других каталогах здесь пока нет.** Жест спрашивает
  /// синхронно, а значения чужих строк живут в ядре: за ними ходят
  /// `Panel.allTargets()`, и обратный вызов для этого должен научиться отдавать
  /// обещание (`docs/spec/drag-and-drop.md`, §10). Место ответа «что поедет»
  /// одно — вот это, — и менять придётся только его.
