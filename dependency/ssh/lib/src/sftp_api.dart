...
abstract interface class SftpApi {
  ...
  /// Пустят ли записать в этот путь прямо сейчас.
  ///
  /// Пробой, а не по битам режима из [SftpEntry.mode]: биты описывают права
  /// владельца, а пишет тот, кем мы вошли на сервер, — ответ по ним выходил бы
  /// то ложным, то пропущенным.
  ///
  /// Открывается на запись **без** `create` и `truncate`: ни файла, ни его
  /// содержимого проба не трогает. Отказ — это `false`, а не исключение: вопрос
  /// как раз о том, откажут ли.
  Future<bool> canWriteTo(String path);
