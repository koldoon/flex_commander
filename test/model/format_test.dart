...
void main() {
  group('formatSize', () {
    ...
    test('килобайты и мегабайты', () {
      expect(formatSize(1024), '1.0K');
      expect(formatSize(6 * 1024), '6.0K');
      expect(formatSize(92262), '90.1K');
      expect(formatSize(15616819), '14.9M');
    });

    test('знак после запятой печатается всегда, даже нулевой', () {
      // Растущая сумма меняется по десять раз в секунду, и на круглом значении
      // число дёргалось формой и шириной: `71.9M` → `72M` → `72.1M`.
      expect(formatSize(2 * 1024 * 1024), '2.0M');
    });

    test('округление переходит к следующей единице', () {
      // 1023.99K округлилось бы до 1024.0K — вместо этого показываем 1.0M.
      expect(formatSize(1024 * 1024 - 1), '1.0M');
    });
