...
void main() {
  ...
  testWidgets('возврат из полноэкранного вида не двигает список', (tester) async {
    // Полноэкранный просмотр панели не закрывает, а **прячет**: возвращаясь,
    // вид строится с нуля. Без памяти о прокрутке он подматывался к курсору
    // заново, и тот оказывался у нижнего края — не там, где его оставили
    ...
    final cursorRow = tester.getRect(find.text(app.left.currentEntry!.name).first);

    app.view.pushViewportContent(ViewportPosition.fullscreen, _FullScreen());
    await tester.pumpAndSettle();
    app.view.popViewportContent(ViewportPosition.fullscreen);
    await tester.pumpAndSettle();

    expect(offsetOf(tester), scrolled);
    expect(tester.getRect(find.text(app.left.currentEntry!.name).first).top, closeTo(cursorRow.top, 0.5));
