...
void main() {
  ...
  test('раздел наборов стоит первым, кто бы ни объявлялся раньше', () async {
    ...
    // нулевой, и стоят они там же, где их объявили. Просят место трое: набор
    // решает всё, что стоит ниже, за ним оформление
    // (`docs/spec/theme-editor.md`, §2), а следом клавиши.
    expect(pages.where((page) => page.priority != 0).map((page) => page.title), [
      'Presets',
      'Themes',
      'Keyboard',
    ], reason: 'место просят набор, оформление и клавиши');
  });
