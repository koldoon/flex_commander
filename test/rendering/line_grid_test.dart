...
void main() {
  ...
  test('интервал остался прежним: шаг равен «кегль × высота» на любом шрифте', () {
    // Сверяется сама величина, а не сравнение с умолчанием библиотеки, как
    // было. `forceStrutHeight` привязывает шаг к кеглю и множителю с точностью
    // до округления — и это ровно то свойство, ради которого делалась починка.
    // А вот прежняя мерка (умолчание апстрима `1.4` без струны) у каждого
    // шрифта своя: на Consolas она давала 21 точку, на Menlo даёт 18. Пока
    // шрифт списка возили с собой, разницы не было видно; теперь он приходит из
    // системы, и сравнивать с величиной, зависящей от шрифта, значило бы
    // проверять шрифт, а не сетку.
    for (final String family in fonts.keys) {
      expect(gridStep(family, textLineHeight), closeTo(fontSize * textLineHeight, 1), reason: family);
    }
  });
}
